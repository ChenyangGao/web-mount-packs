...
.. code: console
    ...
    clouddrive fuse -c '
    import shelve
    cache = shelve.open("clouddrive-cache")'
...
.. code: console
    ...
    clouddrive fuse --direct-open-names vlc

6. 罗列目录时，不走缓存，且每次都刷新

.. code: console

    clouddrive fuse --max-readdir-cooldown 0 --refresh
...
.. code: console
    ...
    clouddrive fuse --strm-predicate '*' --strm-make 'http://my.302.server'

也可以自定义文件的打开目标

.. code: console

    clouddrive fuse --strm-predicate '*' --open-file 'http://my.302.server'

8. 如果你挂载了某个目录，里面都是一些影视剧并且已经刮削好，则只需要再把视频表示为 strm 即可，必要时再把文件的链接进行改写

下面的例子中，clouddrive 挂载了 115 的一个影视剧文件夹 '/115/影视剧'，并且你本地搭建了一个 302 代理服务为 'http://localhost'，然后把所有视频表示为 strm，所有的链接都用 302 代理服务
...
.. code: console
    ...
    clouddrive fuse \\
        ...
        --max-readdir-cooldown 0 # 设定此选项，则总是从 clouddrive 上获取目录列表
...
def main(args):
    ...
    from clouddrive.cmd.fuse.util.fuser import CloudDriveFuseOperations
    from clouddrive.cmd.fuse.util.log import logger
    from clouddrive.cmd.fuse.util.predicate import make_predicate
    from clouddrive.cmd.fuse.util.strm import parse as make_strm_converter
...
parser.add_argument(
    ...
    - module       模块，运行后需要在它的全局命名空间中生成一个 check 或 predicate 函数用于断言，接受一个 clouddrive.CloudDrivePath 对象作为参数
    - file         文件路径，运行后需要在它的全局命名空间中生成一个 check 或 predicate 函数用于断言，接受一个 clouddrive.CloudDrivePath 对象作为参数
...
parser.add_argument(
    ...
    - expr      表达式，可从命名空间访问到一个名为 path 的 clouddrive.CloudDrivePath 对象
    - fstring   视为 fstring，可从命名空间访问到一个名为 path 的 clouddrive.CloudDrivePath 对象
    - lambda    lambda 函数，接受一个 clouddrive.CloudDrivePath 对象作为参数
    - stmt      语句，可从命名空间访问到一个名为 path 的 clouddrive.CloudDrivePath 对象，最后要产生一个名为 url 的变量到本地命名空间
    - module    模块，运行后需要在它的全局命名空间中生成一个 run 或 convert 函数，接受一个 clouddrive.CloudDrivePath 对象作为参数
    - file      文件路径，会被作为模块加载执行，运行后需要在它的全局命名空间中生成一个 run 或 convert 函数，接受一个 clouddrive.CloudDrivePath 对象作为参数
